...
class DataLoader:
            ...
            for parent in parents:
                ...
                if exist_parent:
                    if parent.get("parentIDValue") != exist_parent.get("parentIDValue"):
                        exist_parent["parentIDValue"] = "|".join([exist_parent["parentIDValue"], parent.get("parentIDValue")])
            return True
    ...
    def get_parents(self, relation_fields, rawData):
        ...
        for relation in relation_fields:
            ...
            if val:
                temp = relation.split('.')
                parents.append({"parentType": temp[0], "parentIDPropName": temp[1], "parentIDValue": val.replace(" ", "")})
